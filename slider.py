...
class Slider:
    def __init__(
        ...
        self.x, self.y = (
            rect.x + rect.width * (initial_value / self.range),
            rect.centery,
    ...
    def calculate_slider_value(self) -> None:
        self.value = self.min_value + round(
            (self.x - self.rail.x) * (self.range / self.rect.width),
            self.value_rounding_accuracy,
        )
    ...
    def update(self, mouse_pos, mouse_pressed, cursor, sfx_volume=None) -> None:
        ...
        if not mouse_pressed[0]:
            ...
            if self.clicked:
                ...
                self.x = max(
                    self.rail.x, min(self.x, self.rail.x + self.rail.width)
                )  # if the position eve gets out of bounds, it will get corrected here
    ...
    def draw(self, surf) -> None:
        left_side = pg.Rect(
            self.rect.x, self.rect.y, self.x - self.rect.x, self.rect.height
        )
        right_side = pg.Rect(
            self.x,
            self.rect.y,
            self.rect.width - (self.x - self.rect.x),
            self.rect.height,
        )
